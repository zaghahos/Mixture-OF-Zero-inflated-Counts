...
###################################################################################################
ZINB_mix_K_comp<- function(Y,P,Phi,clust,mu,dispresiopn_parameter,eps=1e-5, maxit=1000){
  dl <- 1 + eps
  iter<-0
  ll <- rep(0, maxit+1)
  G<- ncol(Y)
...
for(s in 1:S){
  ...
  elapsed[s]<- system.time({result_test<- ZINB_mix_K_comp(Y=cell_matrix,P=P,Phi=Phi,mu=MU,clust=clust,dispresiopn_parameter=size,eps=1e-5, maxit=1000)})["elapsed"]
